Hier finden sich alle meine Arduino Schnipsel ein
soweit ich Lust habe werden diese auch hier dokumentiert

+--------------------------------------------------------------+
| 01_433_Send_Simpl                                            |
+--------------------------------------------------------------+
| Kleines Fun Porjekt um meine Funksteckdosen schalten  zu-    |
| koennen.                                                     |
| Getestet auf einem Arduino NANO 328 und billig Sender FS100A |
+--------------------------------------------------------------+
+--------------------------------------------------------------+
| 02_Mini_EthernetShield                                       |
+--------------------------------------------------------------+
| Wie man das Mini Ethernetshield anschliessen muss            |
+--------------------------------------------------------------+
+--------------------------------------------------------------+
| 03_WebHuminity.ino                                           |
+--------------------------------------------------------------+
| Anzeige der aktuellen Temperatur und der Luftfeuchtigkeit    | 
| mittels lokaler HTML Seite                                   |
+--------------------------------------------------------------+
+--------------------------------------------------------------+
| 04_ANAUS.ino                                                 |
+--------------------------------------------------------------+
| Mit diesem Projekt laesst sich ein Ausgang fuer eine         | 
| einstellbare Zeit An und Aus schalten.                       |
| Zur Ansteuerung der 7 Segment wird ein MAX7221 und die       |
| Arduino Lib LedControl                                       |
| Getestet auf Arduino Micro && BS Micro (Leonardo|Atmega 32u4)|
+--------------------------------------------------------------+
+--------------------------------------------------------------+
| 05_SNES_USB.ino                                              |
+--------------------------------------------------------------+
| Code habe ich von                                            |
| https://github.com/cutmore32/                                |
| arduino-leonardo-retro-controller-libraries/blob/master/     |
| snes/snes.ino                                                |
+--------------------------------------------------------------+
| ASCII Controller Port Zeichnung von                          |
| http://davr.org/snes/joystick.txt                            |
+--------------------------------------------------------------+
| Code wurde von mir angepasst auf den BS Micro                |
| Zu Beachten nach erfolgreichen Upload floaten die Eingänge   |
| wenn kein Controller eingesteckt wurde, somit kann es zu     |
| sofortigen Tastatureingaben kommen                           |
| Getestet auf BS Micro                                        |
+--------------------------------------------------------------+

